...
export class Physics {
    constructor(hasPockets = true) {
        ...
        this._left   = TABLE_X + CUSHION_SIZE + BALL_RADIUS;
        this._right  = TABLE_X + TABLE_W - CUSHION_SIZE - BALL_RADIUS;
        this._top    = TABLE_Y + CUSHION_SIZE + BALL_RADIUS;
        this._bottom = TABLE_Y + TABLE_H - CUSHION_SIZE - BALL_RADIUS;
        this.cushionHits = 0;
    ...
    step(balls, dt) {
        ...
        for (let s = 0; s < PHYSICS_SUBSTEPS; s++) {
            for (const b of balls) {
                ...
                const spd = b.speed;
                if (spd > 0.1) {
                    b.rotX += (b.vy / b.radius) * subDt * 2;
                    b.rotY -= (b.vx / b.radius) * subDt * 2;
                }
            }
...
export function castRay(cx, cy, angle, balls, maxLen = 800) {
    ...
    const walls = [
        { x: TABLE_X + CUSHION_SIZE + BALL_RADIUS, nx: 1, ny: 0 },     // left
        { x: TABLE_X + TABLE_W - CUSHION_SIZE - BALL_RADIUS, nx: -1, ny: 0 }, // right
        { y: TABLE_Y + CUSHION_SIZE + BALL_RADIUS, nx: 0, ny: 1 },     // top
        { y: TABLE_Y + TABLE_H - CUSHION_SIZE - BALL_RADIUS, nx: 0, ny: -1 }, // bottom
    ];
